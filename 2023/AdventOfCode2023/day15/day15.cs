...
class Day15()
{
    private static List<string> inputFileLines = Utilites.ReadInputFile("15");

    private static int solvePartOne()
    {
        var steps = inputFileLines[0].Split(',');
        int sum = 0;
    ...
    private static int solvePartTwo()
    {
        ...
        foreach (var step in steps)
        {
            int operationIndex = step.IndexOf('-');
            int focalLength = -1;
            if (operationIndex == -1)
            {
                operationIndex = step.IndexOf('=');
                focalLength = int.Parse(step.Substring(operationIndex + 1));
            }
            if (operationIndex == -1)
            {
                throw new InvalidDataException();
            }
            char operation = step[operationIndex];
