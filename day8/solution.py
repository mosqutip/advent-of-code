...
def parse_instructions(instructions: [str]) -> (int, int):
    accumulator: int = 0
    line_index: int = 0
    visited_lines: {int} = set()

    while line_index < len(instructions):
        if line_index in visited_lines:
...
instructions: [str] = read_input()
print(parse_instructions(instructions)[0])
print(switch_instruction(instructions))
