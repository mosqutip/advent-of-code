...
function getFishCount(numDays: number) {
    ...
    let timerCounts: Array<number> = Array(newFishTimer + 1).fill(0);
    for (let age of timers) {
        timerCounts[age]++;
    }

    for (let i = numDays; i > 0; i--) {
        let zeroTimers = timerCounts[0];
        for (let j = 0; j < newFishTimer; j++) {
            timerCounts[j] = timerCounts[j + 1];
        }

        timerCounts[newFishTimer] = zeroTimers;
        timerCounts[fishTimer] += zeroTimers;
    }

    return timerCounts.reduce((sum, timerCount) => sum + timerCount);
}
