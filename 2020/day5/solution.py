...
def binary_search(key: str) -> (int, int):
    ...
    row_index = max(low_index, high_index)
    ...
    col_index = max(low_index, high_index)

    return row_index, col_index


def find_highest_seat() -> int:
    seats: [str] = parse_input()
    highest_seat_id: int = -1

    for seat in seats:
        row_index, col_index = binary_search(seat)
        seat_id: int = (row_index * 8) + col_index
...
def find_seat() -> int:
    ...
    for seat in seats:
        ...
        # keep a rolling xor calculation of every seen seat ID
        # this is because (a1 ^ a2 ^ ... ^ an) ^ (a1 ^ a2 ^ ... an-1) == an
        visited ^= seat_id
